...
const initSearch = async function (Stringdex, RoaringBitmap, hooks) {
  ...
  function getNextElem(query, parserState, elems, isInGenerics) {
    ...
    const handleRefOrPtr = (chr, name) => {
      ...
      if (parserState.userQuery.slice(pos, pos + 3) === "mut") {
        ...
        chr === "*" &&
        parserState.userQuery.slice(pos, pos + 5) === "const"
      ) {
        parserState.pos += 5;
        c = parserState.userQuery[parserState.pos];
      }
        ...
        if (macroExclamation !== -1) {
          ...
          if (beforeIdent) {
            throw [
              "Unexpected ",
              "!",
              ": it can only be at the end of an ident",
            ];
          }
        }
  ...
  class DocSearch {
    ...
    async getTypeNameIdsAsync(nn) {
        ...
        if (trie) {
          for (const id of trie.matches().entries()) {
            ...
            if (
              pathData &&
              pathData.ty === ty &&
              pathData.modulePath === modulePath
            ) {
              return id;
            }
          }
      ...
      const typeNameIdOfArrayOrSlice = await first(
        arrayOrSlice,
        TY_PRIMITIVE,
        ""
      );
      ...
      const typeNameIdOfTupleOrUnit = await first(
        tupleOrUnit,
        TY_PRIMITIVE,
        ""
      );
    ...
    static parseQuery(userQuery) {
      ...
      try {
        ...
        if (
          Array.isArray(err) &&
          err.every((elem) => typeof elem === "string")
        ) {
          query.error = err;
        } else {
          throw err;
    ...
    async getFunctionData(id) {
      ...
      const parser = new VlqHexDecoder(raw[0], async (functionSearchType) => {
        ...
        if (typeof functionSearchType[INPUTS_DATA] === "number") {
          ...
          inputs_ = this.buildItemSearchTypeAll(
        ...
        if (functionSearchType.length > 1) {
          if (typeof functionSearchType[OUTPUT_DATA] === "number") {
            ...
            output_ = this.buildItemSearchTypeAll(
    ...
    async getRow(id, loadFunctionData) {
      ...
      const buildParentLike = async (field) => {
        const [name, path] =
          entry !== null && entry[field] !== null
            ? await Promise.all([
                this.getName(entry[field]),
                this.getPathData(entry[field]),
              ])
            : [null, null];
      ...
      const [
        ...
        entry && entry.modulePath !== null
          ? this.getName(entry.modulePath)
          : null,
        entry && entry.modulePath !== null
          ? this.getPathData(entry.modulePath)
          : null,
    ...
    async buildItemSearchType(type) {
      ...
      if (id < 0) {
        ...
        result = {
          ...
          exactPath:
            path.exactModulePath === null
              ? path.modulePath
              : path.exactModulePath,
          generics,
          bindings,
    ...
    async execQuery(parsedQuery, filterCrates, currentCrate) {
      const queryLen =
        ...
        parsedQuery.returned.reduce(
          (acc, next) => acc + next.pathLast.length,
          0
        );
      ...
      const formatDisplayTypeSignature = async (
        obj,
        typeInfo,
        elems,
        returned
        ...
        const writeHof = async (fnType, result) => {
          const hofOutput =
            fnType.bindings.get(typeNameIds.typeNameIdOfOutput) || [];
        ...
        const writeSpecialPrimitive = async (fnType, result) => {
          if (
            ...
            if (fnType.generics.length < 2) {
              pushText(
                { name: "const ", highlighted: fnType.highlighted },
                result
              );
            }
            let prevHighlighted = false;
            ...
            const obj = Object.assign(
              {
                parent: item.parent
                  ? {
                      ...
                      exactPath:
                        item.parent.path.exactModulePath ||
      ...
      const sortAndTransformResults = async function* (
        ...
        const transformed_result_list = transformResults(
          result_list,
          typeInfo,
          duplicates
        );
      ...
      const unifyFunctionTypeIsMatchCandidate = (
        fnType,
        queryElem,
        mgensIn,
        typeNameIds
      ...
      function unifyFunctionTypeCheckBindings(
        ...
        if (fnType.bindings.size > 0) {
          ...
          const binds = Array.from(fnType.bindings.entries()).flatMap(
            (entry) => {
      ...
      const checkType = (
        ...
        if (
          ...
          return (
            row.id === elem.id && typePassesFilter(elem.typeFilter, row.ty)
          );
        } else {
      ...
      const innerRunTypeQuery = async function* (
        inputs,
        output,
        typeInfo,
        currentCrate
        ...
        const unpackPostingsList = async (elem, polarity) => {
          ...
          const types = (await Promise.all(typePromises)).filter(
            ([_id, name, ty, path]) =>
              ...
              (elem.pathWithoutLast.length === 0 ||
                checkPath(elem.pathWithoutLast, path.modulePath.split("::")) ===
                  0)
          );
          if (types.length === 0) {
                ...
                for (const id of searchResults.matches().entries()) {
                  ...
                  if (
                    ...
                    if (elem.pathWithoutLast.length !== 0) {
                      ...
                      dist +=
                        pathDist === null ? Number.MAX_SAFE_INTEGER : pathDist;
                    }
                    if (name === chosenName) {
                      chosenId.push(id);
                      chosenType.push(ty);
          ...
          types.sort(
            ([_i, name1, _t, pathData1], [_i2, name2, _t2, pathData2]) => {
          ...
          for (const [id, _name, typeData] of types) {
            ...
            for (const {
              invertedIndex: genericsIdx,
              queryElem: generics,
            } of upla) {
              for (const {
                invertedIndex: bindingsIdx,
                queryElem: bindings,
              } of uplb) {
                results.push({
        ...
        const unpackPostingsListBindings = async (elems, polarity) => {
          ...
          const [firstKeyIds, firstPostingsList, remainingAll] =
            await Promise.all([
          ...
          if (!firstKeyIds) {
            ...
            return [
              { invertedIndex: empty_inverted_index, queryElem: new Map() },
            ];
          }
          const results = [];
        ...
        const queryPlan = [];
        for (const {
          invertedIndex: inputsIdx,
          queryElem: inputs,
        } of allInputs) {
          for (const {
            invertedIndex: outputIdx,
            queryElem: output,
          } of allOutput) {
            const invertedIndex = intersectInvertedIndexes(
              inputsIdx,
              outputIdx
            );
  ...
  function printTab(nb) {
    ...
    onEachLazy(document.getElementById("search-tabs").childNodes, (elem) => {
      if (nb === iter) {
        ...
        onEachLazy(document.querySelectorAll(".search-form"), (form) => {
  ...
  function nextTab(direction) {
    ...
    window.searchState.focusedByTab[searchState.currentTab] =
...
${obj.displayPath}<span class="${type}">${name}</span>\
      ...
      if (obj.displayTypeSignature) {
        const { type, mappedNames, whereClause } =
          await obj.displayTypeSignature;
  ...
  async function showResults(docSearch, results, goToFirst, filterCrates) {
    ...
    nonnull(document.querySelector(".search-switcher")).innerHTML =
      `Search results${crates}`;
    const tabs = [];
    searchState.currentTab = 0;
    if (results.query.error !== null) {
      tabs.push(
        makeTab(0, "In Names", results.others, results.query, false, goToFirst)
      );
    } else if (
      ...
      tabs.push(
        makeTab(1, "In Parameters", results.in_args, results.query, true, false)
      );
      tabs.push(
        makeTab(
          2,
    ...
    if (window.searchState.rustdocToolbar) {
      nonnull(
        nonnull(window.searchState.containerElement()).querySelector(
          ".main-heading"
        )
  ...
  async function search(forced) {
    const query = DocSearch.parseQuery(
      nonnull(window.searchState.inputElement()).value.trim()
    );
