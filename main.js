...
(function () {
  ...
  window.searchState = {
    ...
    inputElement: () => {
      ...
      if (!el) {
        const out = nonnull(
          nonnull(window.searchState.outputElement()).parentElement
        );
    ...
    isDisplayed: () => {
      ...
      return (
        ...
        container.parentElement.id === ALTERNATIVE_DISPLAY_ID
      );
    },
    focus: () => {
    ...
    setup: () => {
      ...
      function loadSearch() {
        if (!searchLoaded) {
          ...
          loadScript(
            getVar("static-root-path") + getVar("stringdex-js"),
            sendSearchForm
          );
  ...
  function showTooltip(e) {
    ...
    if (notable_ty) {
      wrapper.innerHTML =
        '<div class="content">' + window.NOTABLE_TRAITS[notable_ty] + "</div>";
    } else {
...
<a href="${drloChannel}/rustdoc/">the rustdoc book</a>.`;
    ...
    if (isHelpPage) {
      ...
      nonnull(document.getElementById("main-content")).appendChild(
        help_section
      );
    } else {
